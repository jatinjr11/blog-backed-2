...
export const verifyUser = async (req, res) => {
  try {
    ...
    console.log("OTP & TOken : ", otp, activationToken
    );
  ...
  try {
    ...
    if (!user) {
      user = await User.create({
        ...
        email,
        googleId,
        photo: picture,       // Aapka `login` function mein `photo` hai, toh same rakha
        password: "googleuser", // Google user ke liye koi password fixed rakhna
        role: "user",          // Default role set kar sakte hain, ya request se le sakte hain
      });
    }

    // Token create karo aur cookies set karo
